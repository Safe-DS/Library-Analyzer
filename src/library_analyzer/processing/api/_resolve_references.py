...
class ScopeFinder:
    ...
    def detect_scope(self, node: astroid.NodeNG) -> None:
        ...
        for child in self.children:
            if child.parent is not None and child.parent.node != current_scope:
                outer_scope_children.append(child)  # select all children from the outer scope
            else:
                inner_scope_children.append(child)  # select all children from this scope

        self.current_node_stack[-1].children = inner_scope_children  # add all children from this scope to the parent
    ...
    def enter_module(self, node: astroid.Module) -> None:
        self.current_node_stack.append(ScopeNode(node=node, children=None, parent=None))
