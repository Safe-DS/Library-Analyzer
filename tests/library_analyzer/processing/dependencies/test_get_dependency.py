...
def test_extract_action() -> None:
    action_is_ignored = nlp("this parameter is ignored when fit_intercept is set to False.")
    action_is_ignored_action_token = action_is_ignored[3]
    action_is_ignored_condition_token = action_is_ignored[7]

    ignored_action = extract_action(action_is_ignored_action_token, action_is_ignored_condition_token)
    ...
    illegal_action = extract_action(action_is_illegal_action_token, action_is_illegal_condition_token)
    assert illegal_action == ParameterIsIllegal(
        action="Individual weights for each sample raises error base_estimator fit method does not support it",
    )

    action_uncategorised = nlp("If metric is precomputed, X is assumed to be a kernel matrix.")
    action_uncategorised_action_token = action_uncategorised[7]
    action_uncategorised_condition_token = action_uncategorised[3]

    action = extract_action(action_uncategorised_action_token, action_uncategorised_condition_token)
...
def test_extract_condition() -> None:
    ...
    condition_has_value = nlp("this parameter is ignored when fit_intercept is set to False.")
    condition_has_value_root_token = condition_has_value[7]

    has_value_condition = extract_condition(condition_has_value_root_token)
    assert has_value_condition == ParameterHasValue(condition="when fit_intercept is set to False")

    condition_uncategorised = nlp("If metric is a string, it must be one of the metrics in pairwise.")
...
def test_extract_dependencies_from_docstring_pattern_adverbial_clause() -> None:
    ...
    extracted_dependency = DependencyExtractor.extract_pattern_parameter_adverbial_clause(
