...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "to incoming values (weighted 1 - damping). This in order to avoid numerical oscillations when updating "
        ...
        (
            ...
            "must be non-negative.\n\n.. versionchanged: 0.22 The default value of gamma changed from 'auto' to "
            "'scale'.",
            [("float", (0.0, True), ("Infinity", False))]
        ),
        (
        ...
        (
            ...
            'for each class of every column in its own dict. For example, for four-class multilabel classification '
            'weights should be [{0: 1, 1: 1}, {0: 1, 1: 5}, {0: 1, 1: 1}, {0: 1, 1: 1}] instead of [{1:1}, {2:5}, '
            '{3:1}, {4:1}].\n\nThe "balanced" mode uses the values of y to automatically adjust weights inversely '
            'proportional to class frequencies in the input data: n_samples / (n_classes * np.bincount(y)).\n\nFor '
        ...
        (
            ...
            'max_features at random at each split before finding the best split among them. But the best found split '
            'may vary across different runs, even if max_features=n_features. That is the case, if the improvement of '
            'the criterion is identical for several splits and one split has to be selected at random. To obtain a '
            'deterministic behaviour during fitting, random_state has to be fixed to an integer. See :term:Glossary '
        ...
        (
            ...
            "classifiers, or the original one-vs-one ('ovo') decision function of libsvm which has shape (n_samples, "
            "n_classes * (n_classes - 1) / 2). However, note that internally, one-vs-one ('ovo') is always used as a "
            "multi-class strategy to train models; an ovr matrix is only constructed from the ovo matrix. The "
            "parameter is ignored for binary classification.\n\n.. versionchanged: 0.19 decision_function_shape is "
...
def test_extract_boundaries(type_string: str, description: str, expected_boundary: list[BoundaryValueType]) -> None:
    expected = [BoundaryType(
        ...
        min=min_[0],
        max=max_[0],
        min_inclusive=min_[1],
        max_inclusive=max_[1]
