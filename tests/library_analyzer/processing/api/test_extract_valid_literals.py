...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ' If None, estimator is considered fitted if there exist an attribute that ends with a '
            'underscore and does not start with double underscore.',
            ["None", "unlistable_str"],
        ),
        (
            "bool or 'allow-nan'",
            "Whether to raise an error on np.inf, np.nan, pd.NA in X. This parameter does not influence whether y can "
            "have np.inf, np.nan, pd.NA values. The possibilities are: "
        ...
        (
            ...
            'weight is subject to l1/l2 regularization as all other features. To lessen the effect of regularization '
            'on synthetic feature weight (and therefore on the intercept) intercept_scaling has to be increased.',
            [],
        ),
    ],
)
